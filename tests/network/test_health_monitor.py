...
class TestHealthMonitorStats:
    ...
    def test_get_stats_with_routers(self, health_monitor):
        ...
        health_monitor.health_states["r1"] = HealthState(
            status=HealthStatus.HEALTHY, missed_heartbeats=0,
            last_heartbeat=now, last_probe=0, probe_latency_ms=0
        )
        ...
        health_monitor.health_states["r3"] = HealthState(
            status=HealthStatus.WARNING, missed_heartbeats=1,
            last_heartbeat=now - 350, last_probe=0, probe_latency_ms=0
        )
        health_monitor.health_states["r4"] = HealthState(
            status=HealthStatus.DEGRADED, missed_heartbeats=2,
            last_heartbeat=now - 700, last_probe=0, probe_latency_ms=0
        )
        health_monitor.health_states["r5"] = HealthState(
            status=HealthStatus.UNHEALTHY, missed_heartbeats=4,
            last_heartbeat=now - 1500, last_probe=0, probe_latency_ms=0
        )
