...
class TestGossipBroadcast:
    ...
    def test_create_gossip_message(self, node_client):
        ...
        # Add some observations
        now = time.time()
        obs1 = RouterHealthObservation(router_id="r1", latency_ms=50, last_seen=now)
        obs2 = RouterHealthObservation(router_id="r2", latency_ms=75, last_seen=now)
...
class TestRouterSelectionWithGossip:
    ...
    def test_select_router_uses_aggregated_health(self, node_client):
        ...
        node_client._peer_observations["peer1"] = {router2_info.router_id: peer_obs}
