...
def sample_node_trust_row():
    ...
    def _factory(id: UUID | None = None, node_id: UUID | None = None, **kwargs):
        now = datetime.now()
        return {
            "id": id or uuid4(),
...
def sample_federation_node_row():
    ...
    def _factory(id: UUID | None = None, did: str | None = None, **kwargs):
        now = datetime.now()
        return {
            "id": id or uuid4(),
...
class TestSetUserPreference:
    ...
    def test_set_user_preference_with_domain_overrides(self, trust_registry, mock_get_cursor, sample_user_node_trust_row):
        ...
        mock_get_cursor.fetchone.return_value = sample_user_node_trust_row(
            node_id=node_id, domain_overrides={"tech": "elevated", "finance": "reduced"}
        )
