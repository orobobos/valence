-- Migration 005: Verification Protocol with Staking
-- Implements adversarial verification system per spec/components/verification-protocol/
-- Tables: reputations, verifications, disputes, stake_positions, reputation_events, discrepancy_bounties
-- Functions: get_or_create_reputation, lock_stake, release_stake, apply_reputation_update, calculate_belief_bounty
...
-- One active verification per verifier per belief
...
-- FK from verifications.dispute_id to disputes
...
BEGIN
    ...
    IF v_available < p_amount THEN
        RAISE EXCEPTION 'Insufficient available reputation. Available: %, Requested: %',
            v_available, p_amount;
    END IF;

    INSERT INTO stake_positions (
        identity_id, amount, type, verification_id, dispute_id, unlocks_at
    )
    VALUES (
...
-- Apply reputation update with bounds and logging
...
BEGIN
    ...
    -- bounty = holder_stake * bounty_multiplier * confidence^2 * age_factor
    v_bounty := v_holder_stake * 0.5 * (v_confidence * v_confidence) *
                LEAST(2.0, 1.0 + v_age_days / 30.0);
...
-- Auto-create bounty when high-confidence belief is created/updated
...
BEGIN
    ...
    IF v_confidence > 0.5 THEN
        ...
        INSERT INTO discrepancy_bounties (
            belief_id, holder_id, base_amount, confidence_premium, total_bounty
        )
        VALUES (
            NEW.id,
            COALESCE(NEW.holder_id::TEXT, 'local'),
            0.01 * 0.5,
...
-- Update verification count on acceptance
...
COMMENT ON FUNCTION get_or_create_reputation IS 'Get or create reputation for identity';
