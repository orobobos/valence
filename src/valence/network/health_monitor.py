...
class HealthMonitor:
    ...
    def _detect_anomalies(self) -> dict[str, Any] | None:
        ...
        for failure_type, router_ids in failure_type_counts.items():
            ...
            if len(unique_routers) >= self.config.anomaly_threshold:
                ...
                logger.warning(
                    f"ECLIPSE ANOMALY DETECTED: {len(unique_routers)} routers experienced '{failure_type}' within {self.config.anomaly_window}s"
                )
    ...
    def track_ping_response(self, router_id: str, success: bool) -> bool:
        ...
        logger.warning(f"Ping timeout for router {router_id[:16]}... ({missed}/{self.config.missed_pings_threshold} missed)")
