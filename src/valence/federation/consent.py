...
class GatewaySigningProtocol(Protocol):
    ...
    def verify(
        self, data: Dict[str, Any], signature: bytes, gateway_id: str
    ) -> bool:
...
class ConsentChainStoreProtocol(Protocol):
    ...
    async def store_cross_chain(
        self, chain: CrossFederationConsentChain
    ) -> None:
    ...
    async def update_cross_chain(
        self, chain: CrossFederationConsentChain
    ) -> None:
        """Update a cross-federation consent chain."""
        ...
    
    async def store_revocation(
        self, revocation: CrossFederationRevocation
    ) -> None:
    ...
    ) -> Optional[CrossFederationRevocation]:
...
class PolicyStoreProtocol(Protocol):
    ...
    async def get_policy(
        self, federation_id: str
    ) -> Optional[FederationConsentPolicy]:
        """Get consent policy for a federation."""
        ...
    
    async def store_policy(
        self, policy: FederationConsentPolicy
    ) -> None:
    ...
    async def receive_cross_federation_hop(
        ...
        if not is_new_chain:
            ...
            if expected_hop != hop.hop_number:
                raise ValueError(
                    f"Hop number mismatch: expected {expected_hop}, "
                    f"got {hop.hop_number}"
                )
...
class InMemoryConsentChainStore:
    ...
    async def list_pending_revocations(
        ...
        return [
            r for r in self._revocations.values()
...
class MockGatewaySigner:
    ...
    def sign(self, data: Dict[str, Any]) -> bytes:
        ...
        sig = hashlib.sha256(
            (data_json + self._gateway_id).encode()
        ).digest()
