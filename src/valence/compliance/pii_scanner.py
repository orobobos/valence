...
def check_federation_allowed(
    ...
    if result.hard_blocked:
        logger.warning(f"Federation HARD BLOCKED: L4 content detected ({len(result.matches)} PII matches)")
        return False, result
    ...
    if result.max_classification >= ClassificationLevel.L3_PERSONAL:
        if not force:
            ...
            logger.warning(f"Federation forced for L3 content ({len(result.matches)} PII matches)")
