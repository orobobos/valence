...
class ExternalSourceVerificationService:
    ...
    def verify_source(
        ...
        logger.info(f"Source {verification.source_identifier} verified for belief {verification.belief_id} (reliability: {reliability.overall:.2f})")
    ...
    def check_l4_requirements(
        ...
        if not result.has_external_sources:
            result.issues.append(f"Insufficient external sources: {result.total_sources} < {ExternalSourceConstants.MIN_EXTERNAL_SOURCES_FOR_L4}")
            return result
        ...
        if not result.meets_reliability_threshold:
            result.issues.append(f"Best source reliability {best_reliability:.2f} < {ExternalSourceConstants.MIN_VERIFIED_SOURCE_RELIABILITY}")

        if not result.meets_content_match_threshold:
            result.issues.append(f"Best content match {best_content_match:.2f} < {ExternalSourceConstants.MIN_CONTENT_MATCH_SCORE}")
