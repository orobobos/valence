...
def verify_watermark(content: str, secret_key: bytes) -> tuple[bool, Watermark | None]:
...
class WatermarkRegistry:
    ...
    def __repr__(self) -> str:
        ...
        return f"WatermarkRegistry(tokens={len(self._watermarks)}, content_hashes={len(self._content_hashes)})"
    ...
    def investigate_leak(self, leaked_content: str) -> dict | None:
...
_default_registry: WatermarkRegistry | None = None


def get_watermark_registry() -> WatermarkRegistry | None:
