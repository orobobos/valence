...
class CanaryToken:
    ...
    recipient_id: str | None = None  # Who this content was shared with
    content_hash: str | None = None  # Hash of original content for correlation
...
def _decode_from_zero_width(encoded: str) -> str | None:
...
def detect_canaries(content: str, secret_key: bytes | None = None) -> list[CanaryToken]:
...
class LeakReport:
    ...
    context: str | None = None  # Surrounding content
...
class CanaryRegistry:
    ...
    def get_leaks(
        ...
        if token_id:
            leaks = [leak for leak in leaks if leak.token.token_id == token_id]

        if recipient_id:
            leaks = [leak for leak in leaks if leak.token.recipient_id == recipient_id]
