...
def validate_capability(
    ...
    *,
    ...
    if capability.resource != resource:
        errors.append(f"Resource mismatch: capability grants access to '{capability.resource}', but '{resource}' was requested")

    # Check action permission
    if not capability.has_action(action):
        errors.append(f"Action not permitted: capability grants {capability.actions}, but '{action}' was requested")
...
KeyResolver = Callable[[str], Awaitable[Ed25519PublicKey | None]]


async def null_key_resolver(did: str) -> Ed25519PublicKey | None:
...
class CapabilityService:
    ...
    async def issue(
        ...
        parent_capability: Capability | None = None,
        metadata: dict[str, Any] | None = None,
    ) -> Capability:
        ...
        logger.info(f"Issued capability {capability.id}: {issuer_did} -> {holder_did} for {resource} (actions={actions}, ttl={effective_ttl}s)")
